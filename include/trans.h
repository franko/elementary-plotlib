...
namespace graphics { namespace transform {
    ...
    typedef canvas_object_scaling<manage_owner> scaling;
    typedef canvas_object_scaling<manage_not_owner> scaling_a;

    typedef agg::conv_stroke<canvas_object> conv_stroke;

    class stroke_a : public sg_adapter<conv_stroke, approx_scale> {
        typedef sg_adapter<conv_stroke, approx_scale> base_type;
    public:
        stroke_a(canvas_object* src): base_type(src), m_width(1.0) { }
    ...
    struct stroke : stroke_a {
        stroke(canvas_object* src) : stroke_a(src) {}
        virtual ~stroke() {
            delete m_source;
        }
    };
    ...
    struct curve : curve_a {
        curve(canvas_object* src) : curve_a(src) {}
        virtual ~curve() {
            delete m_source;
        }
    };
    ...
    struct dash : dash_a {
        dash(canvas_object* src) : dash_a(src) {}
        virtual ~dash() {
            delete m_source;
        }
    };
    ...
    //------------------------------------------------ affine transform
    class affine_a : public sg_adapter<agg::conv_transform<canvas_object>, no_approx_scale> {
    public:
        affine_a(canvas_object *src, const agg::trans_affine& mtx) : sg_adapter<agg::conv_transform<canvas_object>, no_approx_scale>(src, m_matrix), m_matrix(mtx) {
    ...
    struct affine : affine_a {
        affine(canvas_object* src, const agg::trans_affine& m) : affine_a(src, m) {}
        virtual ~affine() {
            delete m_source;
        }
    };
    ...
    //------------------------------------------------ extend transform
    struct extend : sg_adapter<agg::conv_contour<canvas_object>, approx_scale> {
        extend(canvas_object* src):
            sg_adapter<agg::conv_contour<canvas_object>, approx_scale>(src)
        { }
    ...
    //------------------------------------------------ marker transform
    typedef my::conv_simple_marker<canvas_object, canvas_object> marker_type;
    ...
    public:
        marker_a(canvas_object* src, double size, canvas_object* sym):
    ...
    struct marker : marker_a {
        marker(canvas_object* src, double size, canvas_object* sym):
