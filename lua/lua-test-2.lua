local function fxline(x0, x1, f, N)
    N = N or 512
    local line = elp.Path.new()
    local dx = (x1 - x0) / (N - 1)
    for i = 0, N - 1 do
        local x = x0 + dx * i
        line:LineTo(x, f(x))
    end
    return line
end

local plot = elp.Plot.new()
local x0, x1 = 0.0001, 8 * math.pi * 2
plot:AddStroke(fxline(x0, x1, function(x) return math.sin(x) / x end), 0x0000B4FF, 1.5, elp.property.Stroke)
plot:SetTitle("Function plot example")
plot:SetXAxisTitle("x variable")
plot:SetYAxisTitle("y variable")

local window = elp.Window.new()
window:Attach(plot, "")
window:Start(640, 480, elp.WindowResize)

plot:AddStroke(fxline(0.8, x1, function(x) return math.cos(x) / x end), 0xB40000FF, 1.5, elp.property.Stroke)
window:Wait()
